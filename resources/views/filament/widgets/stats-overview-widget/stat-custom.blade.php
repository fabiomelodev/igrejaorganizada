...
<{!! $tag !!}
    ...
    $getExtraAttributeBag()
        ->class([
            'fi-wi-stats-overview-stat border-b-2 border-primary-600',
        ])
    }}
    >
    <div class="fi-wi-stats-overview-stat-content">
        <div class="fi-wi-stats-overview-stat-label-ctn fi-wi-stats-overview-stat-label-ctn-custom">
            {{ \Filament\Support\generate_icon_html($getIcon()) }}

            <span class="fi-wi-stats-overview-stat-label fi-wi-stats-overview-stat-label-custom">
                {{ $getLabel() }}
            </span>
        </div>
    ...
    @if ($chart = $getChart())
        {{-- An empty function to initialize the Alpine component with until it's loaded with `x-load`. This removes the
